		...
		for (int i = 0; i < handler.getWorld().GridWidthHeightPixelCount; i++) {
			for (int j = 0; j < handler.getWorld().GridWidthHeightPixelCount; j++) {
				...
				if(handler.getWorld().appleLocation[i][j]){
					g.setColor(handler.getWorld().apple.color);
					g.fillOval((i*handler.getWorld().GridPixelsize),
							(j*handler.getWorld().GridPixelsize),
							handler.getWorld().GridPixelsize,
							handler.getWorld().GridPixelsize);
	...
	public void Eat(){
		...
		if(handler.getWorld().apple.isGood()) {
			...
			if(!handler.getWorld().body.isEmpty()) {
				...
				int lastx = handler.getWorld().body.getLast().x;
				int lasty = handler.getWorld().body.getLast().y;

				handler.getWorld().playerLocation[lastx][lasty]=false;
				handler.getWorld().body.removeLast();
			}
			handler.getWorld().appleLocation[xCoord][yCoord]=false;
			handler.getWorld().appleOnBoard=false;
